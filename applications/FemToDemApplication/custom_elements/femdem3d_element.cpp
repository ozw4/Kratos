...
namespace Kratos
{
	...
	void FemDem3DElement::ComputeEdgeNeighbours(ProcessInfo& rCurrentProcessInfo)
	{
		...
		for (int edge = 0 ; edge < 6; edge++)
		{
			...
			for (int i = 0; i < edge_shared_elements_node_2.size(); i++)
			{
				...
				{
					if (edge_shared_elements_node_2[i]->Id() == edge_shared_elements[j]->Id()) aux++;
				}

				if (aux == 0) edge_shared_elements.push_back(edge_shared_elements_node_2[i]);
			}
	...
	)
	{
		...
		if (I1 == 0.0) {
			...
			f = (2.00*std::tan(3.14159265359*0.25 + friction_angle*0.5) / std::cos(friction_angle))*((I1*K3 / 3.0) 
				+ std::sqrt(J2)*(K1*std::cos(theta) - K2*std::sin(theta)*sinphi / std::sqrt(3.0)));
		}

		if (this->GetThreshold(cont) == 0) { this->SetThreshold(c_max, cont); }   // 1st iteration sets threshold as c_max
	...
	) 
	{
		...
		if (this->GetThreshold(cont) == 0.0) { this->SetThreshold(c_max, cont); }   // 1st iteration sets threshold as c_max
		c_threshold = this->GetThreshold(cont);
		this->SetNonConvergedEquivalentStress(f, cont);
