    ...
    def __init__(self, Model, settings ):
        ...
        if settings["active"][0].GetBool() == True:
            ...
            if settings["hydrostatic"].GetBool() == False:
                ...
                if settings["table"][0].GetInt() == 0:
                    ...
                    self.components_process_list.append(KratosGeo.ApplyDoubleTableProcess(model_part, normal_params))
            else:
                ...
                if settings["table"][0].GetInt() == 0:
                    self.components_process_list.append(KratosGeo.ApplyConstantHydrostaticPressureProcess(model_part, normal_params))
                else:
                    normal_params.AddValue("table",settings["table"][0])
                    self.components_process_list.append(KratosGeo.ApplyHydrostaticPressureTableProcess(model_part, normal_params))
        ...
        if settings["active"][1].GetBool() == True:
            ...
            if settings["table"][1].GetInt() == 0:
                ...
                self.components_process_list.append(KratosGeo.ApplyDoubleTableProcess(model_part, tangential_params))
